...
|-----------|-------------|---------------------|
| 全局导航栏 | 模式切换与设置 | 顶部导航栏右侧提供两个独立入口：1. 模式切换按钮（展示当前模式，点击下拉切换学生/教练）；2. 设置按钮（齿轮图标，点击下拉提供“导出备份”和“恢复备份”功能）。 |
...
| 教练日程 (Tab 3 - 教练) | 日程表 | 教练模式专属 Tab 3，图标为 `Calendar`。顶部提供为期 4 周（共 28 天）的横滑日历选择器，默认定位到当天。下方为基于小时和半小时分隔的垂直时间轴。支持点击空白网格快捷创建教案，支持长按拖拽课程卡片更改时间，时间重叠时卡片自适应并排显示。**非核心排课时段（06:00-08:00 及 22:00-24:00）默认折叠，减少无谓滑动；当该时段有排课时自动展开，也支持用户手动点击展开。** |
| 学员管理 (Tab 4 - 教练) | 学员列表 | 教练模式专属 Tab 4，图标为 `Users`。展示所有学员的简要进度和最近上课时间，**支持按水平（1.0-5.0）、具体日期范围或“未上课天数”等多维度组合筛选学员，筛选按钮实时展示准确的筛选结果数量角标。支持左滑删除学员并弹出防误触确认弹窗，删除的学员进入回收站。** |
...
| 学员管理 | 回收站 | 存放被软删除的学员档案，**默认保留 30 天**。支持随时恢复误删的学员，或主动彻底删除以清理数据。 |
...
### 2.6 数据迁移 (Data Migration)
为了方便用户在更换设备时保留所有历史数据，提供极简的导出和导入能力：
- **一键导出**：将所有的打卡记录、备忘录、教练排课、学员档案等打包为一个 JSON 文件，并通过系统原生分享面板（如 AirDrop、微信、文件系统）导出备份。
- **一键导入**：通过系统文件选择器选取导出的 JSON 文件，瞬间覆盖恢复全部状态，实现跨设备无缝迁移。
